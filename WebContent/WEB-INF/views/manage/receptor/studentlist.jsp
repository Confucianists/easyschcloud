...
<body>
	...
	<div class="scorecontentdiv">
		<div class="scoredivtitle">成绩列表/<label id="studentname"></label><a id="scorelistcancel">取消</a></div>
	...
	<div class="studycontentdiv">
		<div class="studydivtitle">成绩列表/<label id="stname"></label><a id="studylistcancel">取消</a></div>
	...
	<link rel="stylesheet" href="${ctx}/static/manage/receptor/css/studentlist.css">
	...
	<script type="text/javascript">
		...
		var pagin;
		var dataurl = "${ctx}/manage/receptor/studentlist";
		$(function() {
			init();
		});
				...
				$("#list")
						.append(
								'<tr><td>'
										...
										+ '</td><td><a aid="'+data.content[i].id
										+'" class="actiona addone">打卡</a><a aid="'+data.content[i].id+'" class="actiona reduceone">撤销</a>'
										+ '</td><td>'
		...
		function initbableaction() {
			...
			$(".addone").bind("click", function() {
				addusecount($(this).attr("aid"));
			});
			$(".reduceone").bind("click", function() {
				reduceusecount($(this).attr("aid"));
			});
		...
		function submit() {
			var saveurl = actiontype == "save" ? "${ctx}/manage/receptor/save"
					: "${ctx}/manage/receptor/update";
			var options = {
				type : 'post',
				url : saveurl,
				dataType : 'json',
		...
		function dodelete(id) {
			$.post('${ctx}/manage/receptor/teacherdel', {
				studentid : id
			}, function(r) {
				if (r.result == "1") {
					pagin.refresh(1);
		...
		function showscorelist(id){
			$("#sclist").empty();
			$.post("${ctx}/manage/receptor/scorereportlist",{studentid:id},function(r){
				if(r.result==1){
		...
		function showstudylist(id){
			$("#stlist").empty();
			$.post("${ctx}/manage/receptor/studyreportlist",{studentid:id},function(r){
				if(r.result==1){
					$("#stname").text(r.name);
					for(var i=0;i<r.data.length;i++){
						$("#stlist").append(
		...
		function submitscore() {
			var saveurl ="${ctx}/manage/receptor/addscorereport";
		...
		function submitstudy() {
			var saveurl ="${ctx}/manage/receptor/addstudyreport";
		...
		function getstudentname1(stid){
			$.post("${ctx}/manage/receptor/getstudentname",{studentid:stid},function(r){
		...
		function addusecount(id){
			$.post("${ctx}/manage/receptor/addusecount",{studentid:id},function(r){
				pagin.refresh();
				showAlert("打卡成功");
			});
		}
		/**撤销打卡*/
		function reduceusecount(id){
			$.post("${ctx}/manage/receptor/reduceusecount",{studentid:id},function(r){
				pagin.refresh();
				showAlert("撤销成功");
			});
		}
