"use strict";
		(function() {
			var detectIEVersion = function() {
				var v = 4, div = document.createElement("div"), all = div
			...
			OssUpload.prototype.init = function(options) {
				...
				if (!options) {
					if (typeof onerror == "function") {
						onerror(errors.format(VODUploadError.CODE.EmptyValue,
								VODUploadError.MESSAGE.EmptyValue, "options"))
					}
					return
				}
				if (!options.file) {
					if (typeof options.onerror == "function") {
						onerror(errors.format(VODUploadError.CODE.EmptyValue,
								VODUploadError.MESSAGE.EmptyValue, "file"))
					}
					return
				}
				if (!options.object) {
					if (typeof options.onerror == "function") {
						onerror(errors.format(VODUploadError.CODE.EmptyValue,
								VODUploadError.MESSAGE.EmptyValue, "object"))
					}
					return
				}
				...
				this._uploadInfo.blobSlice = File.prototype.slice
						|| File.prototype.mozSlice
						|| File.prototype.webkitSlice;
				this._uploadInfo.chunksNum = Math.ceil(options.file.size
				...
				if (err) {
					if (typeof self._callback.onerror == "function") {
						if (err) {
							...
							console
									.log("onMultiUploadComplete: error msg is null.")
						}
						return
					}
					return
			...
			OssUpload.prototype.loadChunk = function() {
				...
				var end = start + config.chunkSize >= _uploadInfo.file.size ? _uploadInfo.file.size
						: start + config.chunkSize;
				var blobPacket = _uploadInfo.blobSlice.call(_uploadInfo.file,
						start, end);
			...
			OssUpload.prototype.frOnerror = function(current, e) {
				...
				var _uploadInfo = self._uploadInfo;
				if (typeof onerror == "function") {
					onerror(errors.format(errors.CODE.ReadFileError,
							errors.MESSAGE.ReadFileError,
							_uploadInfo.file.name, current))
				}
			};
			OssUpload.prototype.resumeUploadWithToken = function(accessKeyId,
			...
			VODUpload.prototype = {
				...
				init : function(accessKeyId, accessKeySecret, securityToken,
						expireTime) {
					if (securityToken && !expireTime || !securityToken
							&& expireTime) {
						return false
					}
					if (accessKeyId && !accessKeySecret || !accessKeyId
							&& accessKeySecret) {
						return false
					}
				...
				addFile : function(file, endpoint, bucket, object, userData) {
					...
					uploadObject.reload = function() {
						if (this.state == VODUpload.UPLOADSTATE.CANCELED
								|| this.state == VODUpload.UPLOADSTATE.FAIlURE) {
				...
				cancelFile : function(index) {
					...
					if (index == options.curIndex
							&& options.uploadList[index].state == VODUpload.UPLOADSTATE.UPLOADING) {
				...
				startUpload : function() {
					...
					if (this.options.state == VODUpload.VODSTATE.START
							|| this.options.state == VODUpload.VODSTATE.EXPIRE) {
						return
					} else if (this.options.state == VODUpload.VODSTATE.STOP) {
						if (options
								&& options.ossUpload
								&& null != options.curIndex
								&& options.uploadList[options.curIndex]
								&& options.uploadList[options.curIndex].state == VODUpload.UPLOADSTATE.STOPED) {
					...
					var endpoint = curObject.endpoint
					...
					ossUpload
							.init({
								...
								onerror : function(evt) {
									if (evt.code == "SecurityTokenExpired"
											|| evt.code == "InvalidAccessKeyId"
											&& options.oss.secretToken
											&& options.oss.secretToken.length > 0) {
								...
								onprogress : function(option) {
									if (options.onUploadProgress) {
										options.onUploadProgress(curObject,
												option.total, option.loaded)
									}
								},
								headers : {
				...
				stopUpload : function() {
					if (this.options.state != VODUpload.VODSTATE.START
							&& this.options.state != VODUpload.VODSTATE.FAILURE) {
						return
					}
					this.options.ossUpload.cancelUpload();
				...
				resumeUploadWithAuth : function(uploadAuth) {
					...
					if (!key.AccessKeyId || !key.AccessKeySecret
							|| !key.SecurityToken || !key.Expiration) {
						return false
					}
					return self.resumeUploadWithToken(key.AccessKeyId,
							key.AccessKeySecret, key.SecurityToken,
							key.Expiration)
				},
				resumeUploadWithToken : function(accessKeyId, accessKeySecret,
					...
					this.init(accessKeyId, accessKeySecret, securityToken,
							expireTime);
					options.ossUpload.resumeUploadWithToken(accessKeyId,
							accessKeySecret, securityToken);
					this.options.state = VODUpload.VODSTATE.START;
					return true
				},
				setUploadAuthAndAddress : function(uploadInfo, uploadAuth,
						uploadAddress) {
					if (!uploadInfo || !uploadAuth || !uploadAddress) {
						return false
					}
					var authKey = JSON
							.parse(ALY.util.base64.decode(uploadAuth));
					if (!authKey.AccessKeyId || !authKey.AccessKeySecret
							|| !authKey.SecurityToken || !authKey.Expiration) {
						return false
					}
					var addressKey = JSON.parse(ALY.util.base64
							.decode(uploadAddress));
					if (!addressKey.Endpoint || !addressKey.Bucket
							|| !addressKey.FileName) {
						return false
					}
